...
generate_parameter_plot <- function(data, condition) { 
# Creates a combined plot, 1 plot for ever parameter_type.
  ...
  #calculates ymin/ymax of mean + sd
    ...
    firstcolour <- ifelse(condition[1]==1, "#FCF3DC80", "#FFFFFF33")# gray60 = speedcondition
    secondcolour <- ifelse(firstcolour == "#FCF3DC80", "#FFFFFF33", "#FCF3DC80")
    rects <- data.frame(xstart = seq(0.5,758.5,48), xend = seq(48.5,768.5,48), 
                        col = rep(c(firstcolour, secondcolour), 8))
...
p1 <- data %>%
  ...
  scale_fill_manual(
    values = c(
    # "firebrick" = alpha("firebrick", 1),  # 
      "firebrick05" = alpha("firebrick", 0.5),  
     # "steelblue" = alpha("steelblue", 1),  
      "868686_with_less_alpha" = alpha("#7f7f7f",.6), #alpha("black", 0.5), #= ifelse(parameter_type=="a", "white", alpha("#868686", 0.5)),  # static model,  inter-trial-var. with alpha=0.5
      "#FCF3DC80" = "#FCF3DC80",  # speedcondition
      "#FFFFFF33" = "#FFFFFF33"  # accuracycondition
    ),
    limits = c(
    #  "firebrick", 
      "firebrick05", 
    #  "steelblue", 
     "868686_with_less_alpha", #can be white inside! 
      "#FCF3DC80", 
      "#FFFFFF33"
    ),
    labels = c(
     # "Mittelw.dynamisches DDM", 
      "SD dynamisches DDM", 
      #"fast-dm Schätzung", 
      ifelse(parameter_type=="a",element_blank(),"SD stationäres DDM"), # Parameter a doesn't have an SD.
 ...
 theme_apa(remove.y.gridlines = F) + 
  ...
  theme(
    ...
    legend.position = if_else(parameter_type=="ndt","bottom","none"),
...
#generate_parameter_plot(data,condition)
...
```{r data resimulation for rt comparison NEW VERSION}
set.seed(123)
person=1 #person
...
for (person in 1:14) { #unique(df$id)
  ...
  df_2 <- df %>% #real data
    ...
    summarize(
      ...
      lower_ci = median_response_time - mad(rt),  #qt(0.975, n() - 1) * sd(rt) / sqrt(n()),
      upper_ci = median_response_time + mad(rt)         #qt(0.975, n() - 1) * sd(rt) / sqrt(n())
    )
  
  combined_summary_all <- simulation_complete_all_14people %>% #for each condition*difficulty*Berechnung of simulated data we want a median and mad of rt
    ...
    summarize(
      ...
      lower_ci = median_response_time - mad(response_time_fake), #qt(0.975, n() - 1) * sd(response_time_fake) / sqrt(n()),
      upper_ci = median_response_time + mad(response_time_fake) #qt(0.975, n() - 1) * sd(response_time_fake) / sqrt(n())
    ) %>%
    bind_rows( #add real data to object
      .,
      df_2)
  
  combined_summary_all$Berechnung <- factor(
    combined_summary_all$Berechnung, 
    levels = c('Real Data', 'BayesFlow', 'STAN')
  )
  
  #plot for all people
  ggplot(data = combined_summary_all, aes(x = factor(difficulty), y = median_response_time, color = Berechnung)) +
    geom_point(size = 3, position = position_dodge(width = 0.6)) +
    geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2, position = position_dodge(width = 0.6)) +
    geom_line(aes(group = Berechnung), position = position_dodge(width = 0.6), size = .5) +
    labs(x = "Schwierigkeit", y = "Reaktionszeit") +
    ggtitle("simulierte und echte Reaktionszeit") +
    scale_color_manual(
      labels = c('emp. Reaktionszeit', 'dynamisches DDM', 'stationäres DDM'),  # Optionally, specify custom labels
    ...
    scale_y_continuous(limits=c(min(combined_summary_all$lower_ci), max(combined_summary_all$upper_ci))) +
    theme_apa() +
...
combined_summary_all <- simulation_complete_all_14people %>%
  mutate(condition = if_else(condition == 0, "Genauigkeitsbedingung", "Geschwindigkeitsbedingung")
  ) %>% 
  filter(
    person == .env$person
...
ggplot(data = combined_summary_all, aes(x = factor(difficulty), y = median_response_time, color = Berechnung)) +
  ...
  theme_apa() +
  facet_wrap(~condition, scales = "free_y")
  ggsave(paste0("C:/Users/steff/Documents/Universität/Master Psychologie/SS 2022/Masterarbeit/Bilder/rt_comparison/MAD/rt_comparison_subj_", person, "_MAD_M1.png"),  width = 6500, height = 4550, units = "px", dpi = 800)
    ggsave(paste0("C:/Users/steff/Documents/Universität/Master Psychologie/SS 2022/Masterarbeit/Bilder/rt_comparison/MAD/rt_comparison_subj_", person, "_MAD_M1.svg"),  width = 6500, height = 4550, units = "px", dpi = 800)
}
